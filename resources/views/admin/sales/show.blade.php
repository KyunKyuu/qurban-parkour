...
<div class="space-y-6">
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <p class="text-[11px] font-semibold uppercase tracking-[0.32em] text-[#a8c5b0]">
                    PIC Kasie · Sales Langsung
                </p>
                <h2 class="display-font mt-3 text-4xl leading-tight text-[#f0ebe0]">{{ $pic->name }}</h2>
                <p class="mt-2 font-mono text-sm text-[#a8c5b0]">{{ $pic->code ?? $pic->email }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
                <span class="rounded-full border border-white/15 px-4 py-2 text-sm font-semibold {{ $pic->is_active ? 'text-emerald-300' : 'text-stone-400' }}">
                    {{ $pic->is_active ? 'Aktif' : 'Nonaktif' }}
                </span>
                <a href="{{ route('admin.sales.index') }}" class="rounded-full border border-white/15 px-5 py-3 text-sm font-semibold text-[#f0ebe0]">
                    ← Kembali
                </a>
                <a href="{{ route('admin.pics.show', $pic->id) }}" class="rounded-full border border-white/15 px-5 py-3 text-sm font-semibold text-[#f0ebe0]">
                    Lihat Profil PJ →
                </a>
            </div>
        </div>
    ...
    <section class="rounded-[1.8rem] border border-[#e5e0d4] bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <p class="text-[11px] font-semibold uppercase tracking-[0.26em] text-stone-400">Peserta Sales Langsung</p>
                <h3 class="mt-1.5 text-lg font-bold text-[#1a3628]">Daftar kontribusi yang masuk langsung via PIC Kasie ini</h3>
            </div>
        </div>
        ...
        <form method="GET" class="mt-5 flex flex-wrap gap-3">
            ...
            @if(request()->hasAny(['category_type', 'search']))
                <a href="{{ route('admin.sales.show', $pic->id) }}" class="rounded-full border border-stone-300 px-5 py-3 text-sm font-semibold text-stone-700">Reset</a>
            @endif
        </form>
        ...
        <div class="mt-5 overflow-x-auto">
            <table class="min-w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-[#f0ebe0]">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="py-8 text-center text-stone-400">Belum ada peserta dari sales langsung ini.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
