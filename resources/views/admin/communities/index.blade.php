...
<div class="space-y-6">
    ...
    <section class="grid gap-4 md:grid-cols-3">
        <div class="rounded-[1.6rem] border border-[#e5e0d4] bg-white p-5 shadow-sm">
            <p class="text-[11px] font-semibold uppercase tracking-[0.26em] text-stone-400">Total Komunitas</p>
            <p class="mt-3 text-3xl font-extrabold text-[#1a3628]">{{ number_format($totalCommunities) }}</p>
        </div>
        <div class="rounded-[1.6rem] border border-[#e5e0d4] bg-white p-5 shadow-sm">
    ...
    <section class="rounded-[1.8rem] border border-[#e5e0d4] bg-white p-6 shadow-sm">
        <form method="GET" class="flex flex-wrap gap-3">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau kode komunitas"
            ...
            @if(request()->hasAny(['pic_id', 'search']))
                <a href="{{ route('admin.communities.index') }}" class="rounded-full border border-stone-300 px-5 py-3 text-sm font-semibold text-stone-700">Reset</a>
            @endif
        </form>
    </section>
            ...
            <table class="min-w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-stone-100">
                    ...
                    <tr class="hover:bg-stone-50 transition">
                        <td class="px-6 py-4">
                            <p class="font-semibold text-[#1a3628]">{{ $community->name }}</p>
                            <p class="text-xs text-stone-400 font-mono mt-0.5">{{ $community->code }}</p>
                        </td>
                        <td class="px-6 py-4 text-stone-600">{{ $community->pic?->name ?? '-' }}</td>
                        <td class="px-6 py-4 font-semibold text-stone-900">{{ number_format($community->voucher_count) }}</td>
                        <td class="px-6 py-4">
                            <span class="font-semibold text-stone-900">{{ number_format($community->registered_count) }}</span>
                            @if($community->voucher_count > 0)
                            <span class="text-xs text-stone-400 ml-1">/ {{ $community->voucher_count }}</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 font-semibold text-[#1a3628]">Rp {{ number_format($community->total_amount, 0, ',', '.') }}</td>
                        <td class="px-6 py-4">
                            <span class="inline-flex rounded-full px-3 py-1 text-[11px] font-semibold {{ $community->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-100 text-stone-500' }}">
                                {{ $community->is_active ? 'Aktif' : 'Nonaktif' }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <a href="{{ route('admin.communities.show', $community->id) }}"
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-6 py-10 text-center text-stone-400">Belum ada komunitas yang ditemukan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
