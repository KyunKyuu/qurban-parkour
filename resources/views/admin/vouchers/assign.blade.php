...
<div class="max-w-2xl space-y-6">
    ...
    <div>
        <h2 class="text-2xl font-bold text-emerald-950">Assign Voucher ke PIC Komunitas</h2>
        <p class="text-emerald-600 text-sm">Voucher hanya dapat di-assign langsung ke PIC Komunitas.</p>
    </div>
        ...
        <form method="POST" action="{{ route('admin.vouchers.assign') }}" class="space-y-5">
            ...
            <div>
                ...
                <select name="pic_id" id="pic_id" required
                    ...
                    <option value="">-- Pilih PIC Komunitas --</option>
                ...
                <p class="mt-1 text-xs text-stone-400">Komunitas otomatis mengikuti PIC Komunitas yang dipilih.</p>
                @error('pic_id')
            ...
            @if($picKomunitas->isEmpty())
                <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                    Belum ada PIC Komunitas yang aktif.
                    <a href="{{ route('admin.pics.create') }}" class="underline font-semibold">Tambah PIC Komunitas</a>
                </div>
            @endif
